...
const OwnerDashboard = () => {
  ...
  useEffect(() => {
    const unsubscribers = BRANCHES.map(({ id }) => {
      ...
      return onSnapshot(q, (snap) => {
        ...
        // archived field ممكن مايبقاش موجود في الأوردرات الجديدة
        const orders = allOrders.filter((o) => o.archived !== true);
  ...
  const analyzeProducts = useCallback(async () => {
    ...
    allOrders.forEach((order) => {
      (order.items || []).forEach((item) => {
        ...
        salesMap[key].totalRevenue +=
          (item.price_single || 0) * (item.qty || 1);
      });
    });
    setTopProducts(
  ...
  const handleSaveWeeklyReport = async () => {
    if (!window.confirm("هتحفظ تقرير الأسبوع وتعمل reset للداشبورد. متأكد؟"))
      return;
    setSaving(true);
    try {
      const now = new Date();
      ...
      // احفظ التقرير
      await setDoc(
        doc(db, "weeklyReports", "data", "reports", reportId),
        reportData,
      );
      ...
      await Promise.all(
        BRANCHES.map(async ({ id }) => {
          const snap = await getDocs(
            query(collection(db, id, "orders", "data")),
          );
          snap.docs.forEach((d) => {
            const data = d.data();
    ...
    try {
      ...
      await setDoc(doc(db, "weeklyReports", "data", "reports", reportId), {
        ...
        branches: {
          mansoura: {
            name: "المنصورة",
            revenue: Number(manualForm.mansoura) || 0,
            total: 0,
            delivered: 0,
          },
          mit_ghamr: {
            name: "ميت غمر",
            revenue: Number(manualForm.mit_ghamr) || 0,
            total: 0,
            delivered: 0,
          },
          zagazig: {
            name: "الزقازيق",
            revenue: Number(manualForm.zagazig) || 0,
            total: 0,
            delivered: 0,
          },
        },
  ...
  const handleAddTarget = async () => {
    if (!targetForm.assignee || !targetForm.targetValue) {
  ...
  // ── Totals ────────────────────────────────────────────────
  const totalRevenue = Object.values(branchStats).reduce(
    (s, b) => s + (b.revenue || 0),
    0,
  );
  const totalOrders = Object.values(branchStats).reduce(
    (s, b) => s + (b.total || 0),
    0,
  );
  const totalPending = Object.values(branchStats).reduce(
    (s, b) => s + (b.pending || 0),
    0,
  );
  const totalPreparing = Object.values(branchStats).reduce(
    (s, b) => s + (b.preparing || 0),
    0,
  );
  const totalInProgress = totalPending + totalPreparing;
  const totalDelivered = Object.values(branchStats).reduce(
    (s, b) => s + (b.delivered || 0),
    0,
  );
  ...
  const topPayment = Object.entries(paymentStats).sort(
    (a, b) => b[1] - a[1],
  )[0];
    ...
    <div className="min-h-screen bg-gray-900 text-white" dir="rtl">
      ...
      <div className="bg-gray-800 px-6 py-4 flex justify-between items-center sticky top-0 z-10 border-b border-gray-700">
        ...
        <div className="flex items-center gap-3 flex-wrap justify-end">
          ...
          <Link
            to="/owner/reports"
            className="bg-blue-600 px-4 py-2 rounded-lg text-sm hover:bg-blue-500 transition-colors font-bold"
          >
            📋 التقارير
          </Link>
          {/* حفظ يدوي */}
          ...
          <button
            ...
            className="bg-red-600 px-4 py-2 rounded-lg text-sm hover:bg-red-500 transition-colors"
          >
            خروج
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6">
        <StatCard
          ...
          value={`${totalRevenue.toFixed(2)} ج`}
      ...
      <div className="px-6 pb-10">
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                    ...
                    <MiniStat
                      label="الإيرادات"
                      value={`${(stats.revenue || 0).toFixed(2)} ج`}
                      highlight
                    />
                  </div>
                  <div className="mb-4">
                  ...
                  {(stats.recentOrders || []).length === 0 ? (
                    ...
                    <div className="flex flex-col gap-2">
                      {stats.recentOrders.map((o, i) => (
                        <div
                          key={i}
                          className="flex justify-between items-center bg-gray-700 rounded-lg px-4 py-3 text-sm"
                        >
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-400 text-xs">
                          {p.totalRevenue.toFixed(2)} ج إيرادات
                        </p>
                      </div>
                      <div className="hidden md:flex flex-col items-end gap-1 w-32">
                        ...
                        <div className="w-full bg-gray-600 rounded-full h-1.5">
                          <div
                            ...
                            style={{
                              width: `${Math.min((p.totalQty / (topProducts[0]?.totalQty || 1)) * 100, 100)}%`,
                            }}
                          />
                        </div>
                      </div>
                      <span className="text-yellow-400 font-black text-sm md:hidden">
                        {p.totalQty}x
                      </span>
                    </div>
                  ))}
              ...
              {slowProducts.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {slowProducts.map((p, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 bg-gray-700 rounded-xl px-4 py-3 border border-orange-500/10"
                    >
                      <div className="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 bg-gray-600 flex items-center justify-center">
                        {p.image?.startsWith("http") ? (
                          <img
                      ...
                      <div className="text-center flex-shrink-0">
                        <p
                          className={`font-black text-lg ${p.sales === 0 ? "text-red-400" : "text-orange-400"}`}
                        >
                          {p.sales}
                        </p>
                      ...
                      return (
                        <div key={method}>
                          ...
                          <div className="w-full bg-gray-700 rounded-full h-2.5">
                            <div
                              className="h-2.5 rounded-full bg-gradient-to-r from-yellow-500 to-orange-500 transition-all duration-700"
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                        </div>
            ...
            <div className="bg-gray-800 rounded-2xl p-5 border border-gray-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-gray-700 rounded-xl p-4 text-center">
                  ...
                  {(() => {
                    const top = BRANCHES.map(({ id, name }) => ({
                      name,
                      revenue: branchStats[id]?.revenue || 0,
                ...
                <div className="bg-gray-700 rounded-xl p-4 text-center">
                  ...
                  {topPayment ? (
                    <>
                      <p className="text-white font-black text-lg">
                        {paymentLabels[topPayment[0]] || topPayment[0]}
                      </p>
                      <p className="text-yellow-400 font-bold">
                        {topPayment[1]} أوردر
                      </p>
                ...
                <div className="bg-gray-700 rounded-xl p-4 text-center">
                  ...
                  <p className="text-white font-black text-lg">
                    {totalOrders > 0
                      ? (totalRevenue / (totalDelivered || 1)).toFixed(2)
                      : "0.00"}{" "}
                    ج
                  </p>
            ...
            <div className="bg-gray-800 rounded-2xl p-5 border border-gray-700">
              ...
              <div className="flex flex-col gap-3">
                {slowProducts.length > 0 && (
                  <Alert
                    type="warning"
                    message={`${slowProducts.length} منتج بيعهم بطيء — فكر في عروض أو تخفيضات عليهم`}
                  />
                )}
                {totalPending > 5 && (
                  <Alert
                    type="danger"
                    message={`في ${totalInProgress} أوردر نشط — تأكد إن الفروع شايفينهم`}
                  />
                )}
                {completionRate < 70 && totalOrders > 10 && (
                  <Alert
                    type="warning"
                    message={`نسبة إتمام الأوردرات ${completionRate}% — أقل من المتوسط المطلوب`}
                  />
                )}
                {topProducts[0] && (
                  <Alert
                    type="success"
                    message={`"${topProducts[0].name}" هو الأكثر مبيعاً — ${topProducts[0].totalQty} قطعة اتباعت`}
                  />
                )}
                {slowProducts.length === 0 &&
                  totalPending <= 5 &&
                  completionRate >= 70 && (
                    <Alert
                      type="success"
                      message="كل حاجة تمام! 🎉 المبيعات شغالة كويس"
                    />
                  )}
              </div>
            </div>
        ...
        {activeTab === "targets" && (
          <div className="flex flex-col gap-6">
            <div className="bg-gray-800 rounded-2xl p-5 border border-gray-700">
              <h2 className="text-lg font-bold text-yellow-400 mb-4">🎯 إضافة تارجت جديد</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <input className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white" placeholder="اسم الموظف" value={targetForm.assignee} onChange={(e) => setTargetForm((p) => ({ ...p, assignee: e.target.value }))} />
                <select className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white" value={targetForm.branchId} onChange={(e) => setTargetForm((p) => ({ ...p, branchId: e.target.value }))}>
                  {BRANCHES.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
                </select>
                <select className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white" value={targetForm.type} onChange={(e) => setTargetForm((p) => ({ ...p, type: e.target.value }))}>
                  <option value="orders">عدد الأوردرات</option>
                  <option value="revenue">الإيراد</option>
                </select>
                <input type="number" className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white" placeholder="قيمة التارجت" value={targetForm.targetValue} onChange={(e) => setTargetForm((p) => ({ ...p, targetValue: e.target.value }))} />
                <select className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white" value={targetForm.rewardType} onChange={(e) => setTargetForm((p) => ({ ...p, rewardType: e.target.value }))}>
                  <option value="bonus">مكافأة مالية</option>
                  <option value="discount">خصم خاص</option>
                </select>
                <input type="number" className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white" placeholder="قيمة المكافأة/الخصم" value={targetForm.rewardValue} onChange={(e) => setTargetForm((p) => ({ ...p, rewardValue: e.target.value }))} />
              </div>
              <button onClick={handleAddTarget} className="mt-4 bg-yellow-500 text-gray-900 font-bold px-5 py-2 rounded-lg">حفظ التارجت</button>
            </div>

            <div className="bg-gray-800 rounded-2xl p-5 border border-gray-700">
              <h2 className="text-lg font-bold text-yellow-400 mb-4">📋 سجل التارجت</h2>
              <div className="space-y-3">
                {targets.length === 0 ? (
                  <p className="text-gray-500">لا يوجد تارجت حتى الآن</p>
                ) : targets.map((t) => (
                  <div key={t.id} className="bg-gray-700 rounded-lg p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div>
                      <p className="text-white font-bold">{t.assignee} - {BRANCHES.find((b) => b.id === t.branchId)?.name || t.branchId}</p>
                      <p className="text-gray-400 text-sm">النوع: {t.type === "revenue" ? "إيراد" : "أوردرات"} | الهدف: {t.targetValue}</p>
                      <p className="text-gray-400 text-sm">المكافأة: {t.rewardType === "bonus" ? "Bonus" : "Discount"} - {t.rewardValue}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-1 rounded-full ${t.status === "completed" ? "bg-green-500/20 text-green-400" : "bg-orange-500/20 text-orange-400"}`}>
      ...
      {showManualModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-md border border-gray-700">
            <h2 className="text-lg font-bold text-yellow-400 mb-5">
              ✏️ إضافة تقرير يدوي
            </h2>
            <div className="flex flex-col gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setManualForm((p) => ({ ...p, label: e.target.value }))
                  }
                />
              </div>
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setManualForm((p) => ({ ...p, [key]: e.target.value }))
                      }
                    />
                  </div>
                ))}
...
const StatCard = ({ label, value, color, textColor }) => (
  <div
    className={`bg-gray-800 rounded-xl p-5 border-r-4 ${color} border border-gray-700`}
  >
...
const MiniStat = ({ label, value, highlight, color }) => (
  <div className="bg-gray-700 rounded-lg p-3 text-center">
    ...
    <p
      className={`text-lg font-bold ${highlight ? "text-yellow-400" : color || "text-white"}`}
    >
      {value}
    </p>
  </div>
...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    rejected: { label: "مرفوض ❌", cls: "bg-red-800 text-red-300" },
  };
